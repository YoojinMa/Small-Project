...
public class UsedItemController {
	...
	public static void updateUsedItemPrice(String usedItemId, String usedItemPrice) {
		try {
			...
			RunEndView.showError("현재 ID로 물건 가격 변경 오류. 재시도 하세요!");
		} catch (NotExistException e) {
			RunEndView.showError("현재 ID 확인 후 재시도 하세요!");
		}
	}
	...
	public static void updateUsedItemStatus(String usedItemId, String dealStatus) {
		try {
			...
			RunEndView.updateView(r, usedItemId);
			Log.itemStatusUpdateCheck("Success", usedItemId, dealStatus);
		} catch (SQLException e) {
			RunEndView.showError("현재 ID로 물건 거래 상태 변경 오류. 재시도 하세요!");
		} catch (NotExistException e) {
	...
	public void createMenu() {
		...
		while (!finish) {
			...
			try {
				...
				if (number == 0) {
					finish = true;
					System.out.println("■ ■ ■ 중고거래 검색을 종료합니다:) ■ ■ ■");
				}
			} catch (Exception e) {
		...
		try {
			...
			RunEndView.showError("■ ■ ■ 정확하게 입력하세요:) ■ ■ ■");
		}
	}
